...
def show_district_mapping(df_2021):
    ...
    regional_stats = df_2021_with_region.groupby('Region')[['Overall_Performance', 'Math_Performance', 'Science_Performance', 'SST_Performance', 'Language_Performance']].agg(['mean', 'std', 'count']).round(2)
...
def show_insights_and_recommendations(df_2021):
    ...
    insights = [
        ...
        },
        {
            ...
            "content": f"""
            - **Top Performing State/UT:** {top_state} ({state_performance.iloc[0]:.1f}%)
    ...
    with col1:
        ...
        if len(low_performing_districts) > 0:
            worst_districts = low_performing_districts.nsmallest(10, 'Overall_Performance')[['State', 'District', 'Overall_Performance']]
    ...
    with col2:
        ...
        if len(high_performing_districts) > 0:
            best_districts = high_performing_districts.nlargest(10, 'Overall_Performance')[['State', 'District', 'Overall_Performance']]
    ...
    for rec in recommendations:
        with st.expander(rec["category"], expanded=False):
    ...
    for i, (state, performance) in enumerate(consistent_performers.items(), 1):
        ...
        st.markdown(f"""
        **{i}. {state}** - {performance:.1f}% Overall Performance
    ...
    with col2:
        if st.button("📈 Download District Analysis"):
            district_summary = df_2021[['State', 'District', 'Overall_Performance', 'Math_Performance', 'Science_Performance', 'SST_Performance', 'Language_Performance']]
